declare module 'posthtml' {
  ...
  export type PostHTMLNodeCallback = (
    node: PostHTMLNode
  ) => MaybeArray<PostHTMLNode>
  ...
  export interface PostHTMLNode<T = string, A = PostHTMLNodeAttributes> {
    ...
    match: (
      expression: PostHTMLExpression,
      cb: PostHTMLNodeCallback
    ) => PostHTMLNode[]
    tag?: T
    attrs?: A
  ...
  export type PostHTMLPlugin<T> = (
    tree: PostHTMLNode
  ) => void | PostHTMLNode | ThisType<T>
  ...
  export default function posthtml<T, M>(
    plugins?: PostHTMLPlugin<T>[]
  ): PostHTML<T, M>
}
